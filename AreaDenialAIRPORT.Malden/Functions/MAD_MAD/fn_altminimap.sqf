...
switch (toUpper _mode) do
{
	...
	{	
		...
		//vehicle player addEventHandler ["FiredMan", {params ["_unit", "_weapon", "_muzzle", "_mode", "_ammo", "_magazine", "_projectile", "_gunner"]; player allowDamage true; player setvariable['MFired', true, false]; if !(vehicle _unit inArea t1) then { ['HINT', MAD_HUD_hintBASTARD,[1, 0.2, 0, 1]] spawn MAD_fnc_altminiMap;}; }];
		...
		_minimap ctrlAddEventHandler ["Draw", {['DRAW', _this select 0] call MAD_fnc_altminiMap}];
	...
	case 'DRAW':
	{
		...
		if (time> MAD_HUD_HINT_TIMER) then
		{
			...
			['HINT', MAD_HUD_HINT,[0, 0, 0, 0.7]] spawn MAD_fnc_altminiMap;
		};
		
		_ECount=0; _WCount=0;
		_ECountT=0; _WCountT=0;		
		{		
             _side= _x getVariable ['_S',side _x];		
			
			if (_side isEqualTo EAST) then 
			{
				if (_x inArea t1 and alive _x) then { _ECount= _ECount +1};
				_ECountT= _ECountT+1;
			};
		...
		if (time> MAD_HUD_HINT_TIMER_INFO) then
		{
			...
			_txt = format["<a align='center' size='1' color='#FFAA00'> <img  shadow= '0' color='#AAAAAAFF' image='\A3\Data_F\Flags\Flag_red_CO.paa'/> %1 <a align='center' size='0.7' color='#FFFFFF'> %2 </a></a><a align='center' size='1' color='#00BBFF'> %3 <img  shadow= '0' color='#EEFFFFFF' image='\A3\Data_F\Flags\Flag_blue_CO.paa'/> </a> ",	
			...
			//_txt = format["<a align='center' size='1' color='#FFAA00'>%1 <img  shadow= '0' color='#AAAAAAFF' image='\A3\Data_F\Flags\Flag_red_CO.paa'/> %2 <a align='center' size='0.7' color='#FFFFFF'> %3 </a></a><a align='center' size='1' color='#00BBFF'> %4 <img  shadow= '0' color='#EEFFFFFF' image='\A3\Data_F\Flags\Flag_blue_CO.paa'/> %5 </a> ",	
				//_ECountT, _ECount, localize "STR_MADINAREA",  _WCount, _WCountT];// west vs east
				
				 _ECount, localize "STR_MADINAREA",  _WCount];// west vs east
				
				
				
				//_RCountT, _RCount, localize "STR_MADINAREA",  _WCount, _WCountT];  // west vs resistance
		...
		} forEach ((Allunits- [player]) select {alive _x and (_x getVariable ['_S', side _x]) isEqualTo S3_Side});	
