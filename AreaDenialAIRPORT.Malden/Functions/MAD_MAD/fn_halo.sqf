...
if (typename _this == typename objnull) then {
	...
	//_dir = ([[0,0,0],velocity _unit] call bis_fnc_dirto);
	 ...
	 _dir =([player, t_ao] call BIS_fnc_dirTo);
	_unit setdir _dir;
	...
	if (_unit == player) then {
		...
		if !(isnil 'bis_fnc_halo_action') then {player removeaction bis_fnc_halo_action; bis_fnc_halo_action= nil};
		
		bis_fnc_halo_action = _unit addAction [format["<t size='1.2' color='#EEEEFF'><img size='1.5' shadow= '0' image='A3\ui_f\data\Map\VehicleIcons\iconParachute_ca.paa'/> %1 </t>", localize "STR_PARACHUTE1"],{[player] call MAD_fnc_halo}, 0,6, true, true, "Eject"];
		//bis_fnc_halo_action = _unit addaction [localize "STR_HALO_OPEN_CHUTE","A3\functions_f\misc\fn_HALO.sqf",[],16,true,true,"Eject"];
		...
		*/
		...
		[] spawn {
			...
			while {alive player && vehicle player == player && isnil {player getvariable "bis_fnc_halo_terminate"}} do {
				...
				bis_fnc_halo_velLimit = 0.5 * _fpsCoef;
				bis_fnc_halo_velAdd = 0.015 * _fpsCoef;
...
if (typename _this == typename []) then {
	...
	if (_unit == player) then {
		...
		waituntil {(position vehicle player select 2) < .25};
		
		//player allowDamage false;	
	//	_landpos= getPosASL player;
	//	systemChat  str _landpos;
	//	player setPosASL _landpos;
	//	systemChat "landed";
	//	player allowDamage true;	
