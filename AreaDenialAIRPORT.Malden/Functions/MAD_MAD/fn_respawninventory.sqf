...
//if (isNil "Respawnready") exitWith { diag_log "notready inventory";};
disableserialization;
//systemChat "respawninventory";
...
if !(alive player) then {		//player dead, show respawn menu
	...
	{	
		...
		_color= _combo lbColor (lbCurSel _combo);
		//systemchat str (_color);
		if (_color isEqualTo [1,1,1,1]) then
		{	
			//Default
			//systemchat 'Default Config loadout';
		...
		{	
			...
			waitUntil {!isNil 'MADcustomInventoryReady'};
			[player,[profileNamespace, _NAME_LOADOUT_MAD]] call MAD_fnc_loadinventory;	
			//systemChat ('CUSTOM LOADOUT: '+ _NAME_LOADOUT_MAD); 
		};
		//*/
	};
