...
if (_unit getVariable ["BIS_fnc_showRespawnMenu_disable", false]) exitWith {diag_log "exit";};
...
if !(alive player) then
{
	...
	["enable",uiNamespace getVariable ["BIS_RscRespawnControlsMap_ctrlLocList", controlNull],0,localize "STR_A3_RscRespawnControls_UnitUnavailable", [player] call BIS_fnc_objectVar] call BIS_fnc_showRespawnMenuDisableItem;
...
else
{
	...
	if (isNil {uiNamespace getVariable ["BIS_RscRespawnControlsMap_ctrlLocList", controlNull]}) then
	{
	...
	else
	{
		...
		_list = uiNamespace getVariable ["BIS_RscRespawnControlsMap_ctrlLocList",controlNull];

		if (missionNamespace getVariable ["BIS_RscRespawnControlsSpectate_shown", false]) then {_list = uiNamespace getVariable ["BIS_RscRespawnControlsSpectate_ctrlLocList", controlNull]};

		_curSel = if !(lbCurSel _list < 0) then {lbCurSel _list} else {0};

		if !(isNil {uiNamespace getVariable ["BIS_RscRespawnControls_posMetadata",controlNull]}) then
		{
