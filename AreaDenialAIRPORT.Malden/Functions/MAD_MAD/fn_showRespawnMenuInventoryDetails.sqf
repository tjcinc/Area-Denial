...
with uiNamespace do {
					...
					missionNamespace setVariable ["BIS_RscRespawnControls_detailsHandlerDisplay", _display displayAddEventHandler ["mouseButtonDown",
					{
						[] spawn 
						{
							...
							{	//no control handler in last 0.2s, which means the click was outside the control and the control should be closed
								["close"] spawn MAD_fnc_showRespawnMenuInventoryDetails;
							};
						};
					}]];
				};
	...
	{	
		...
		if (isnull _previewObject) then {_previewObject = (typeof player) createvehiclelocal [0,0,0]; };
		//waitUntil {!(isnull _previewObject)};
		//hint format['CUSTOMTEST: %1 %2 ',_previewObject,_NAME_LOADOUT_MAD];
		//_previewObject=player;	
		
		//[_previewObject,[profileNamespace, _NAME_LOADOUT_MAD]] call MAD_fnc_loadinventory;
		
		
		//_previewObject setUnitLoadout "B_Survivor_F";
		//_loadout= [_previewObject, [profileNamespace,_NAME_LOADOUT_MAD]] call MAD_fnc_loadinventory;	
		//_loadout= [_previewObject, west_sleader3] call BIS_fnc_loadinventory;	
		//_loadout=  [_previewObject,configfile >> "CfgVehicles" >> "B_Soldier_SL_F"] call BIS_fnc_loadInventory;
		...
		_NAME_LOADOUT_MAD= _combo lbText (lbCurSel _combo);
		};

		//systemchat format['NO Metadata for item found, process it a CUSTOM %1 ', _NAME_LOADOUT_MAD];	
		//systemchat format['%1 %2 %3', _previewObject, _NAME_LOADOUT_MAD, missionNamespace getvariable ['AAS_profile',[]]];	
		

		waitUntil {!isNil 'MAD_fnc_loadInventory'};
		[_previewObject,[profileNamespace,_NAME_LOADOUT_MAD]] call MAD_fnc_loadInventory;
		
		//systemchat ('MAD_fnc_loadinventory');
		...
		//hint format['primaryweapon _previewObject: %1 ',primaryweapon _previewObject];
	...
	if (count _this==1) then
	{
		...
		[_previewObject,_cfg] call bis_fnc_loadInventory;
		//systemchat format['TEST: %1 %2',_previewObject,_cfg];
	...
	_fnc_list = {	//processing all items of loadout and fill them in the list
		...
		if !(_curSelCombo < 0) then {	//there is something selected
			...
			{
				//[player,[profileNamespace, 'TUBERMAN 2']] call MAD_fnc_loadinventory;		
				['wtf',0] call _fnc_addItems;	
	...
	switch (_mode) do {
		case "auto": {	//--- auto mode - detects current state and changes it
		    //player sidechat format['AUTO T0 %1', BIS_RscRespawnControls_details];
			if ( ctrlFade _ctrlGroup==0) then {//BIS_RscRespawnControls_details
			...
			BIS_RscRespawnControls_details= not BIS_RscRespawnControls_details; 
		};
		case "open": {	//--- open mode - opens details, doesn't care what is the current status
			//player sidechat 'OPEN';
			_ctrlGroup ctrlEnable true;
			_ctrlGroup ctrlSetFade 0;
			_ctrlGroup ctrlCommit 0;
